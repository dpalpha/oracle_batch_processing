...
<<PLSQL_BLOCK>>
    ...
    <<batch_processing>>
    open tab;
    fetch tab bulk collect into lvid limit 50;
    forall i in lvid.FIRST .. lvid.LAST
        insert into RES select t1.TRID, t2.LID from (select * from TRANSACTIONS where TRID in lvid(i)) t1 join TRANSA t2 on t1.TRID=t2.TRID;
